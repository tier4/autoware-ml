...
def test_crossing_living_agent_is_finite_within_horizon() -> None:
    ego = Agent.wheeled(0.0, 0.0, heading=0.0, speed=10.0, body_radius=1.0)
    ped = Agent.living(18.0, 6.0, speed=4.0, body_radius=0.4)
...
def test_steps_stay_within_horizon() -> None:
    ...
    living = Agent.living(7.6, 0.0, speed=5.0, body_radius=0.5)
...
def test_ego_reachability_matches_bruteforce_stepping() -> None:
    ...
    for index in range(60):
        ...
        if kind == AgentKind.STATIC:
            ...
            obj = Agent.living(x, y, speed=float(rng.uniform(0.0, 6.0)), body_radius=0.4)
        else:
            obj = Agent.wheeled(
                x,
                y,
...
def test_an_object_beside_ego_collides_at_the_first_step() -> None:
    ...
    ego = Agent.wheeled(0.0, 0.0, heading=0.0, speed=10.0, body_radius=1.2)
    ped = Agent.living(0.1, 0.55, speed=1.4, body_radius=0.4)
...
def test_a_stopped_agent_still_occupies_its_body() -> None:
    stopped = Agent.wheeled(20.0, 0.0, heading=0.0, speed=0.0, body_radius=1.5)
...
def test_a_stopped_body_stays_on_its_own_side_of_a_median() -> None:
    ...
    median = box(-100.0, -4.0, 200.0, 0.0).union(box(-100.0, 0.5, 200.0, 4.5))
    ego = Agent.wheeled(0.0, -2.0, heading=0.0, speed=10.0, body_radius=1.2)
    parked = Agent.wheeled(40.0, 0.7, heading=pi, speed=0.0, body_radius=1.2)
...
def test_a_stopped_agent_off_the_surface_has_no_body() -> None:
    ...
    parked = Agent.wheeled(20.0, 70.0, heading=0.0, speed=0.0, body_radius=1.0)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        {"kind": AgentKind.WHEELED, "speed": float("nan"), "body_radius": 1.0},
        {"kind": AgentKind.WHEELED, "heading": float("nan"), "speed": 1.0, "body_radius": 1.0},
        {
        ...
        {"kind": AgentKind.STATIC, "speed": 0.0, "body_radius": 1.0, "footprint": Polygon()},
        {"kind": "wheeled", "speed": 10.0, "body_radius": 1.0},
    ],
)
def test_an_unusable_agent_is_rejected(kwargs: dict) -> None:
    with pytest.raises(ValueError):
        Agent(**{"x": 0.0, "y": 0.0, "heading": 0.0, "speed": 0.0, "body_radius": 1.0, **kwargs})
