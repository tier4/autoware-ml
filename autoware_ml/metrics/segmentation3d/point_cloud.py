            ...
            if self._component_filters and active:
                ...
                for index, metric_filter in enumerate(self._component_filters):
                    ...
                    if context is None:
                        ...
                        box_columns.append(np.zeros(boxes_np.shape[0], dtype=bool))
                        continue
                    # xyz only: a wide per-point array must never trip the filters'
                    # box-vs-point dispatch. Boxes go through the same filter as full
                    # rows, footprint-overlap membership.
