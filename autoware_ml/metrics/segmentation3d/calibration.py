...
def _ece_from_bins(conf_sum: np.ndarray, correct_sum: np.ndarray, count: np.ndarray) -> float:
    """Expected calibration error from confidence bins.

    The count-weighted sum over the non-empty bins of the gap between accuracy and mean
    confidence, NaN when no bin holds a point.

    Args:
        conf_sum: Confidence summed per bin.
        correct_sum: Correct predictions summed per bin.
