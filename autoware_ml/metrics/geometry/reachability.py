...
class Agent:
    ...
    static kind ignores both and uses ``footprint``. ``front`` and ``rear`` are
    the body's extents ahead of and behind the reference point along the heading
    and ``half_width`` its extent across, so a collision is a body overlap rather
    than a point coincidence. The reference point is wherever the pose is
    measured: a detection box is centred, ego is its rear axle, so the two
    extents are not assumed equal.
    ...
    def __post_init__(self) -> None:
        ...
        extents = (self.front, self.rear, self.half_width)
        if not all(isfinite(value) for value in extents):
            raise ValueError("front, rear and half_width must be finite.")
    ...
    def body_reach(self) -> float:
        ...
        The farthest body corner lies within ``max(front, rear) + half_width``
        of the reference point, so this is the padding every distance bound
        needs to stay a superset.
        """
        return max(self.front, self.rear) + self.half_width


@dataclass(frozen=True)
class VehicleGeometry:
    """Ego body in Autoware's vehicle description terms, measured from the rear axle.
    ...
    def front(self) -> float:
        """Distance from the rear axle to the front bumper in meters."""
        return self.wheel_base + self.front_overhang

    @property
    def rear(self) -> float:
        """Distance from the rear axle to the rear bumper in meters."""
...
def _arc_poses(agent: Agent, distances: np.ndarray, kappas: np.ndarray) -> np.ndarray:
    """Poses ``(x, y, heading)`` at the signed arc lengths ``distances`` along each arc.

    One arc per signed curvature in ``kappas`` (positive turns left of the
    heading), all starting at the agent's pose. Negative distances travel in
    reverse along the same circle, the heading staying the vehicle's front. The
    map frame is right-handed in (x, y). Shape ``(curvature, pose, 3)``, one
    vectorized evaluation because a front samples the whole curvature by length
    grid at every step of every object.
    """
    kappa = np.asarray(kappas, dtype=np.float64).reshape(-1, 1)
    s = np.asarray(distances, dtype=np.float64).reshape(1, -1)
    ...
    phi = s * kappa  # signed swept angle
    dx, dy = agent.x - cx, agent.y - cy
    x_curved = cx + dx * np.cos(phi) - dy * np.sin(phi)
    y_curved = cy + dx * np.sin(phi) + dy * np.cos(phi)
    x = np.where(straight, x_straight, x_curved)
    y = np.where(straight, y_straight, y_curved)
    heading = np.broadcast_to(agent.heading + phi, x.shape)
    return np.stack((x, y, heading), axis=-1)


def _body_polygons(agent: Agent, poses: np.ndarray) -> np.ndarray:
    """The agent's body rectangle at every pose of ``poses`` (``(pose, 3)``)."""
    corners = np.array(
        [
            (agent.front, -agent.half_width),
            (agent.front, agent.half_width),
            (-agent.rear, agent.half_width),
            (-agent.rear, -agent.half_width),
        ]
    )
    cos_h = np.cos(poses[:, 2:3])
    sin_h = np.sin(poses[:, 2:3])
    x = poses[:, 0:1] + corners[:, 0] * cos_h - corners[:, 1] * sin_h
    y = poses[:, 1:2] + corners[:, 0] * sin_h + corners[:, 1] * cos_h
    return shapely.polygons(np.stack((x, y), axis=-1))


def _connected_sweep(
    body: BaseGeometry, drivable: BaseGeometry, anchor: BaseGeometry
) -> BaseGeometry:
    """``body`` clipped to the surface, keeping only the parts that touch ``anchor``.

    Clipping alone would let the body hang over a non-drivable gap narrower than
    itself and re-land on a disconnected carriageway, so the parts of the clipped
    body that do not touch the driven path are dropped. The path is on the
    surface while the body may overhang it.
    """
    parts = _polygonal_parts(body.intersection(drivable))
    return shapely.union_all([part for part in parts if part.intersects(anchor)])


def _drivable_poses(poses: np.ndarray, seed: Point, drivable: BaseGeometry) -> np.ndarray | None:
    """The prefix of ``poses`` the agent can actually drive, from its own position.

    An arc that leaves the road is truncated there rather than discarded, so a
    lane that merely brushes a kerb keeps the travel before it, and the pose at
    the road edge is appended so the body stops at the edge rather than one
    step short of it. ``None`` when there is no travel at all, because the
    reference point is off the surface or the arc leaves it at once.
    """
    path = LineString(poses[:, :2])
    if drivable.covers(path):
        return poses
    parts = [
        part
        for part in shapely.get_parts(path.intersection(drivable))
        if part.geom_type == "LineString" and not part.is_empty
    ]
    driven = next((part for part in parts if part.distance(seed) <= SURFACE_TOLERANCE_M), None)
    if driven is None or driven.length <= SURFACE_TOLERANCE_M:
        return None
    along = np.concatenate(([0.0], np.cumsum(np.hypot(*np.diff(poses[:, :2], axis=0).T))))
    last = int(np.searchsorted(along, driven.length + SURFACE_TOLERANCE_M, side="right")) - 1
    kept = poses[: last + 1]
    if along[last] < driven.length - SURFACE_TOLERANCE_M:
        # The edge lies on the segment after the last kept pose, at this fraction of it.
        fraction = (driven.length - along[last]) / (along[last + 1] - along[last])
        kept = np.vstack((kept, poses[last] + fraction * (poses[last + 1] - poses[last])))
    return kept


def _curvature_count(agent: Agent, reach: float, kmax: float, minimum: int) -> int:
    """Odd number of curvature samples whose sweeps still overlap at their far ends.

    Neighbouring arcs end about ``reach^2 * kmax / (count - 1)`` apart, and the
    body corners at their ends twice ``extent * reach * kmax / (count - 1)``
    further apart because the two bodies point along different tangents. The
    count is raised until that corner spacing is within the body width, otherwise
    the outer edge of the union is left with uncovered notches between
    neighbouring sweeps, a hole a narrow body opens even at the default count.
    """
    extent = max(agent.front, agent.rear)
    spacing = reach * kmax * (reach + 2.0 * extent)
    count = max(minimum, ceil(spacing / (2.0 * agent.half_width)) + 1)
    return count + (1 - count % 2)


def _body_sweep(agent: Agent, poses: np.ndarray, drivable: BaseGeometry) -> BaseGeometry:
    """The body at every pose of the driven path, clipped and kept connected to it."""
    body = shapely.union_all(_body_polygons(agent, poses))
    return _connected_sweep(body, drivable, LineString(poses[:, :2]))
...
def wheeled_reachable_set(
    ...
    The sweep places the body rectangle at one pose per time step along the arc,
    so its outer corners are covered through the whole turn and a shorter
    travel's poses are a prefix of a longer one's. The set therefore grows with
    ``t`` by construction, and the same call at the horizon is the region the
    prescreen and the collision filter rely on.
    ...
    # The body at the start pose, clipped the same way: an agent that cannot move,
    # or whose every arc leaves the surface at once, still owns it.
    start = np.array([[agent.x, agent.y, agent.heading]])
    pieces = [_connected_sweep(_body_polygons(agent, start)[0], drivable, seed)]
    reach = agent.speed * t
    ...
    kappas = np.linspace(
        -kmax, kmax, _curvature_count(agent, horizon_reach, kmax, params.arc_samples)
    )
    steps = int(t / params.dt_s + 1e-9)
    distances = np.arange(steps + 1) * agent.speed * params.dt_s
    if reach - distances[-1] > SURFACE_TOLERANCE_M:
        distances = np.append(distances, reach)
    # Forward and reverse: the worst case is whichever direction closes the gap.
    for sign in (1.0, -1.0):
        for poses in _arc_poses(agent, sign * distances, kappas):
            driven = _drivable_poses(poses, seed, drivable)
            if driven is not None:
                pieces.append(_body_sweep(agent, driven, drivable))
...
def reachable_set(
    ...
    Wheeled fronts are truncated where their arc leaves ``drivable``, so a
    wheeled evaluation needs a drivable polygon. Living discs and static
