...
class CollisionWeightedMeanAP(Metric[DetectionState]):
    ...
    def __init__(
        ...
        """Validate the match thresholds and the risk decay.

        Args:
            thresholds: Center-distance match thresholds in meters, the weighted AP is averaged
                over them.
            decay: Exponential risk decay per second of TTC, finite and non-negative.
        ...
        if not self.thresholds or any(threshold <= 0.0 for threshold in self.thresholds):
            raise ValueError("thresholds must name at least one distance > 0.")
        if not isfinite(self.decay) or self.decay < 0.0:
