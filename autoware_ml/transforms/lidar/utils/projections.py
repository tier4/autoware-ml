...
def project_spherical(points, w, h, dilation_size: int):
    ...
    if np.any(initial_valid_mask):
        ...
        if dilation_size > 0:
            ...
            broadcasted_intensities = np.broadcast_to(valid_intensities[:, np.newaxis], patch_rows.shape)
...
def create_lidar_image(h: int, w: int, camera_matrix, max_depth: float, dilation_size: int,
        points_xyz: npt.NDArray[np.float32], intensities: npt.NDArray[np.float32]
    ) -> npt.NDArray[np.float32]:
