...
becomes its on-road or on-walkway variant. The bundled configs define the road slice as the union
of the lanelet primitives road, road_shoulder, crosswalk, drivable_area, intersection_area and
crosswalk_polygon, and the walkway slice as the walkway primitive.
...
filter, computed per frame from the same collision model the collision metrics use. A detection
box counts when any part of its footprint meets the area, a segmentation point when it lies
inside. Only ego is propagated here and every element is tested where it stands; the objects'
own reachable sets enter through the collision metrics, not through the filter.
...
The model answers one question: if ego never sees this object, what is the worst that can
follow? Every agent moves at its class maximum speed under its tightest turn, forward or in
reverse, so the collision set is a bound, not a prediction. A lead at matched speed is not exempt,
it may brake. A wheeled body is swept as a rectangle placed at every time step along the arc,
clipped to the drivable area and reduced to the part still connected to the driven path. The
rectangle sits around the pose's reference point: a detection box is centred, ego is measured at
its rear axle.
...
<svg viewBox="0 0 700 210" width="700" height="210" role="img" aria-labelledby="fig-lead-title">
      <title id="fig-lead-title">A lead vehicle at the same speed: ego keeps advancing to the right while the lead may brake or reverse, so the collision sets grow towards each other and meet after about one second - matched speed gives no protection.</title>
      ...
      <!-- ego forward reach and lead reverse reach, growing towards each other -->
      <rect x="98" y="118" width="150" height="28" rx="6" fill="var(--ego)" opacity="0.18"/>
      <rect x="248" y="119" width="112" height="26" rx="6" fill="var(--obj)" opacity="0.18"/>
      <!-- t=now: ego + lead -->
      <rect x="52" y="120" width="46" height="24" rx="4" fill="var(--ego)"/>
      <text x="75" y="160" class="svg-label small" text-anchor="middle">ego (t)</text>
      <rect x="360" y="121" width="42" height="22" rx="4" fill="var(--obj)"/>
      <text x="381" y="160" class="svg-label small" text-anchor="middle">lead (t)</text>
      <!-- ego drives on -->
      <path d="M 104 132 L 240 132" stroke="var(--ego)" stroke-width="1.6" stroke-dasharray="5 4"/>
      <path d="M 232 127 L 242 132 L 232 137" fill="none" stroke="var(--ego)" stroke-width="1.6"/>
      <text x="172" y="94" class="svg-label small" text-anchor="middle" opacity="0.85">ego keeps going</text>
      <!-- lead brakes or reverses -->
      <path d="M 354 132 L 256 132" stroke="var(--obj)" stroke-width="1.6" stroke-dasharray="5 4"/>
      <path d="M 264 127 L 254 132 L 264 137" fill="none" stroke="var(--obj)" stroke-width="1.6"/>
      <text x="305" y="94" class="svg-label small" text-anchor="middle" opacity="0.85">lead brakes or reverses</text>
      <!-- meeting point -->
      <line x1="248" y1="108" x2="248" y2="156" stroke="var(--danger)" stroke-width="2"/>
      <text x="248" y="184" class="svg-label small" text-anchor="middle" fill="var(--danger)">sets meet: TTC ~ 1 s</text>
      <!-- the same road, seen from above, is what a following ego actually risks -->
      <rect x="540" y="121" width="42" height="22" rx="4" fill="var(--obj)" opacity="0.28"/>
      <text x="561" y="94" class="svg-label small" text-anchor="middle" opacity="0.75">lead at matched speed (t+1)</text>
      <text x="350" y="40" class="svg-label strong" text-anchor="middle">matched speed is no protection -> the gap closes at the sum of both speeds</text>
    </svg>
</div>
...
<svg viewBox="0 0 660 232" width="660" height="232" role="img" aria-labelledby="fig-b2-scene-title">
        <title id="fig-b2-scene-title">Bird's-eye view: ego on a two-lane road. An oncoming vehicle closes fast (low time-to-collision, high weight); a parked car beyond the horizon reach can never be met (infinite time-to-collision, weight zero).</title>
        ...
        <!-- parked car, same lane, beyond the horizon reach -->
        <line x1="470" y1="70" x2="470" y2="174" stroke="var(--muted)" stroke-width="1.5" stroke-dasharray="6 5" opacity="0.7"/>
        <text x="470" y="62" class="svg-label small" text-anchor="middle">horizon reach</text>
        <rect x="520" y="136" width="44" height="26" rx="4" fill="var(--obj)"/>
        <text x="542" y="184" class="svg-label small" text-anchor="middle">parked, 70 m ahead</text>
